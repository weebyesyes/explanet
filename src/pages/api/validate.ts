...
async function runPythonValidate(csvPath: string): Promise<PythonResult> {
  ...
  return new Promise((resolve, reject) => {
    ...
    proc.on("close", (code) => {
      ...
      try {
        ...
        reject(new Error(`Unable to parse validator output: ${(err as Error).message}\n${stdout}`));
      }
    });
  });
}
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const result = await runPythonValidate(csvPath);
